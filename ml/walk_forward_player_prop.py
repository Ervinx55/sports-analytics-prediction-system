...
def aggregate_by_market(predictions: pd.DataFrame) -> dict:
    report = {}
    for stat_id, part in predictions.groupby("stat_id", sort=True):
        report[str(stat_id)] = {
            "rows": int(len(part)),
            "events": int(part["event_id"].nunique()),
            "metrics": aggregate_predictions(part),
        }
    return report
...
def main() -> None:
    ...
    by_market = aggregate_by_market(predictions)
    ...
    for stat_id, market_report in by_market.items():
        market_metrics = market_report["metrics"]
        summary.extend([
            "",
            f"### {stat_id}",
            f"- Rows: {market_report['rows']}",
    ...
    compact = {
        ...
        "by_market": {
            stat_id: {
                ...
                "champion_brier":
                    market_report["metrics"]["champion"]["brier"],
                "ensemble_brier":
